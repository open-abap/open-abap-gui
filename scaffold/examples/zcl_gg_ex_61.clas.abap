CLASS zcl_gg_ex_61 DEFINITION PUBLIC FINAL CREATE PUBLIC.

* Feature 61, active, inactive, and excluded application commands.
...
CLASS zcl_gg_ex_61 IMPLEMENTATION.
  METHOD zif_gg_transaction_v1~get_transaction.
    rs_transaction = VALUE #( tcode = 'ZGG_EX_61' description = 'Active inactive and excluded commands' ).
  ENDMETHOD.

  METHOD zif_gg_report_v1~start_of_selection.
    io_session->get_list( )->set_title( 'ZCL_GG_EX_61' ).
  ...
  METHOD zif_gg_list_processing_v1~at_user_command.
    IF iv_ucomm = 'ENABLE'.
      io_session->get_list( )->get_writer( )->write_field( VALUE #( text = 'enabled' placement = VALUE #( new_line = abap_true ) ) ).
    ENDIF.
  ENDMETHOD.
