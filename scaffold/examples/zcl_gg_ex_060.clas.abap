CLASS zcl_gg_ex_060 DEFINITION PUBLIC FINAL CREATE PUBLIC.

* Feature 60, ordered icon separators and accessible labels.
...
CLASS zcl_gg_ex_060 IMPLEMENTATION.
  METHOD zif_gg_transaction_v1~get_transaction.
    rs_transaction = VALUE #( tcode = 'ZGG_EX_060' description = 'Icon separators and accessibility' ).
  ENDMETHOD.
  ...
  METHOD zif_gg_list_processing_v1~at_user_command.
    CASE iv_ucomm.
      WHEN 'FIRST'.
        io_session->get_list( )->get_writer( )->write_field( VALUE #( text = 'first' placement = VALUE #( new_line = abap_true ) ) ).
      WHEN 'SECOND'.
        io_session->get_list( )->get_writer( )->write_field( VALUE #( text = 'second' placement = VALUE #( new_line = abap_true ) ) ).
      WHEN 'PRI'.
