...
CLASS ltcl_ex_59 IMPLEMENTATION.
  METHOD owns_refresh_and_print.
    DATA(ls_result) = zcl_gg_host=>run( NEW zcl_gg_ex_59( ) ).
  ...
  METHOD command_authorization.
    ...
    APPEND VALUE #( report = NEW zcl_gg_ex_59( ) ucomm = 'REFR' ) TO lt_cases.
    APPEND VALUE #( report = NEW zcl_gg_ex_60( ) ucomm = 'FIRST' ) TO lt_cases.
    APPEND VALUE #( report = NEW zcl_gg_ex_61( ) ucomm = 'ENABLE' ) TO lt_cases.
    APPEND VALUE #( report = NEW zcl_gg_ex_62( ) ucomm = 'NEXT' ) TO lt_cases.
    APPEND VALUE #( report = NEW zcl_gg_ex_66( ) ucomm = 'RUN66' ) TO lt_cases.
