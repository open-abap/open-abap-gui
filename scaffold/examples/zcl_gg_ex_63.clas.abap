CLASS zcl_gg_ex_63 DEFINITION PUBLIC FINAL CREATE PUBLIC.

* Feature 63, declared PF keys and an explicit rejected key.
...
CLASS zcl_gg_ex_63 IMPLEMENTATION.
  METHOD zif_gg_transaction_v1~get_transaction.
    rs_transaction = VALUE #( tcode = 'ZGG_EX_63' description = 'Declared PF5 and rejected PF6' ).
  ENDMETHOD.

  METHOD zif_gg_report_v1~start_of_selection.
    io_session->get_list( )->set_title( 'ZCL_GG_EX_63' ).
  ...
  METHOD zif_gg_list_processing_v1~at_pf.
    IF iv_key = 5.
      io_session->get_list( )->get_writer( )->write_field( VALUE #( text = 'pf5' placement = VALUE #( new_line = abap_true ) ) ).
    ENDIF.
  ENDMETHOD.
