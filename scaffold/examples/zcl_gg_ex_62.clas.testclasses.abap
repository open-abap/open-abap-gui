...
CLASS ltcl_ex_62 IMPLEMENTATION.
  METHOD changes_status_after_next.
    ...
    DATA(ls_start) = zcl_gg_host_runtime=>start( io_report = NEW zcl_gg_ex_62( ) ).
    cl_abap_unit_assert=>assert_equals( act = ls_start-current_page-status-status exp = 'SHELL62' ).
    ...
    cl_abap_unit_assert=>assert_equals( act = ls_next-current_page-status-status exp = 'SHELL62-DONE' ).
