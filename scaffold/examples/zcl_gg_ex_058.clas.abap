...
* zgg_ex_058.prog.abap. The HTML host drives this as the /ZCL_GG_EX_058 route.
...
CLASS zcl_gg_ex_058 IMPLEMENTATION.

  METHOD zif_gg_transaction_v1~get_transaction.
    rs_transaction = VALUE #( tcode = 'ZGG_EX_058' description = 'SET SCREEN, LEAVE SCREEN and LEAVE TO SCREEN' ).
  ENDMETHOD.
  ...
  METHOD zif_gg_dynpro_v1~build_screens.
    io_builder->begin_screen( VALUE #( number = '0100' title = 'ZCL_GG_EX_058' ) ).
    io_builder->end_screen( ).
    io_builder->begin_screen( VALUE #( number = '0200' title = 'ZCL_GG_EX_058' ) ).
    io_builder->end_screen( ).
  ENDMETHOD.
