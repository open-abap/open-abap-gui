...
CLASS ltcl_gg_rich_list_base IMPLEMENTATION.
  ...
  METHOD drill_down.
    ...
    lo_report = NEW zcl_gg_ex_83( ).
    DATA(ls_detail) = zcl_gg_host=>run( io_report = lo_report iv_line_index = 1 ).
    cl_abap_unit_assert=>assert_true( act = xsdbool( line_exists( ls_detail-lines[ table_line = 'Detail list' ] ) ) ).
    DATA(ls_subdetail) = zcl_gg_host=>run( io_report = lo_report iv_line_index = 2 ).
    cl_abap_unit_assert=>assert_true( act = xsdbool( line_exists( ls_subdetail-lines[ table_line = 'Subdetail list' ] ) ) ).
  ENDMETHOD.

  METHOD hidden_values.
    DATA(ls_result) = zcl_gg_host=>run( io_report = NEW zcl_gg_ex_84( ) iv_line_index = 2 ).
    cl_abap_unit_assert=>assert_true( act = xsdbool( line_exists( ls_result-lines[ table_line = 'selected bravo' ] ) ) ).
    cl_abap_unit_assert=>assert_equals( act = ls_result-render_lines[ 2 ]-fields[ name = 'SECRET' ]-value exp = 'bravo' ).
  ENDMETHOD.

  METHOD refresh.
    DATA(ls_result) = zcl_gg_host=>run( io_report = NEW zcl_gg_ex_85( ) iv_user_command = 'REFRESH' ).
    cl_abap_unit_assert=>assert_true( act = xsdbool( line_exists( ls_result-lines[ table_line = 'refreshed from server state' ] ) ) ).
    cl_abap_unit_assert=>assert_equals( act = ls_result-status-status exp = 'REFRESHED' ).
  ENDMETHOD.

  METHOD modify_lines.
    DATA(ls_result) = zcl_gg_host=>run( io_report = NEW zcl_gg_ex_86( ) iv_user_command = 'MODIFY' ).
    ...
    cl_abap_unit_assert=>assert_equals( act = lines( ls_result-render_lines[ 1 ]-fragments ) exp = 2 ).
    cl_abap_unit_assert=>assert_equals( act = ls_result-render_lines[ 1 ]-fields[ name = 'ROW' ]-value exp = '1' ).
  ENDMETHOD.

  METHOD fragments.
    DATA(ls_result) = zcl_gg_host=>run( io_report = NEW zcl_gg_ex_87( ) ).
    cl_abap_unit_assert=>assert_equals( act = lines( ls_result-render_lines[ 1 ]-fragments ) exp = 3 ).
    cl_abap_unit_assert=>assert_true( act = ls_result-render_lines[ 1 ]-fragments[ 1 ]-format-intensified ).
    cl_abap_unit_assert=>assert_true( act = ls_result-render_lines[ 1 ]-fragments[ 3 ]-format-inverse ).
  ENDMETHOD.

  METHOD icons.
    DATA(ls_result) = zcl_gg_host=>run( io_report = NEW zcl_gg_ex_88( ) ).
    cl_abap_unit_assert=>assert_equals( act = ls_result-render_lines[ 1 ]-fragments[ 1 ]-kind exp = 'ICON' ).
    cl_abap_unit_assert=>assert_equals( act = ls_result-render_lines[ 1 ]-fragments[ 2 ]-kind exp = 'SYMBOL' ).
    cl_abap_unit_assert=>assert_equals( act = ls_result-render_lines[ 1 ]-fragments[ 3 ]-kind exp = 'CHECKBOX' ).
  ENDMETHOD.

  METHOD columns.
    DATA(ls_result) = zcl_gg_host=>run( io_report = NEW zcl_gg_ex_89( ) ).
  ...
  METHOD unicode.
    DATA(ls_result) = zcl_gg_host=>run( io_report = NEW zcl_gg_ex_90( ) ).
  ...
  METHOD page_breaks.
    DATA(ls_result) = zcl_gg_host=>run( io_report = NEW zcl_gg_ex_91( ) ).
  ...
  METHOD paging.
    DATA lo_report TYPE REF TO zif_gg_report_v1.
    lo_report = NEW zcl_gg_ex_92( ).
    DATA(ls_result) = zcl_gg_host=>run( io_report = lo_report iv_user_command = zif_gg_session_types_v1=>command_next_page ).
    cl_abap_unit_assert=>assert_equals( act = ls_result-status-status exp = 'PAGE 2' ).
    cl_abap_unit_assert=>assert_true( act = xsdbool( line_exists( ls_result-lines[ table_line = 'Flight 4' ] ) ) ).
  ENDMETHOD.

  METHOD find_next.
    DATA(ls_result) = zcl_gg_host=>run( io_report = NEW zcl_gg_ex_93( ) iv_user_command = zif_gg_session_types_v1=>command_find ).
    cl_abap_unit_assert=>assert_true( act = xsdbool( line_exists( ls_result-lines[ table_line = 'Found LH at row 1' ] ) ) ).
  ENDMETHOD.

  METHOD print_view.
    DATA(ls_result) = zcl_gg_host=>run( io_report = NEW zcl_gg_ex_94( ) iv_user_command = 'PRINT_VIEW' ).
    cl_abap_unit_assert=>assert_true( act = xsdbool( ls_result-lines[ 2 ] CS 'PRINT VIEW' ) ).
  ENDMETHOD.

  METHOD download.
    DATA(ls_result) = zcl_gg_host=>run( io_report = NEW zcl_gg_ex_95( ) iv_user_command = 'DOWNLOAD' ).
    cl_abap_unit_assert=>assert_true( act = xsdbool( ls_result-lines[ 4 ] CS 'flights.csv' ) ).
  ENDMETHOD.

  METHOD messages.
    DATA(ls_result) = zcl_gg_host=>run( io_report = NEW zcl_gg_ex_96( ) ).
    cl_abap_unit_assert=>assert_equals( act = lines( ls_result-messages ) exp = 2 ).
    cl_abap_unit_assert=>assert_equals( act = ls_result-messages[ 1 ]-type exp = zif_gg_session_types_v1=>message_type_success ).
    cl_abap_unit_assert=>assert_equals( act = ls_result-messages[ 2 ]-type exp = zif_gg_session_types_v1=>message_type_warning ).
  ENDMETHOD.

  METHOD list_memory.
    DATA(ls_result) = zcl_gg_host=>run( io_report = NEW zcl_gg_ex_97( ) ).
    cl_abap_unit_assert=>assert_equals( act = ls_result-navigation-kind exp = 'SUBMIT_RETURN' ).
  ENDMETHOD.

  METHOD workbench.
    DATA(ls_result) = zcl_gg_host=>run( io_report = NEW zcl_gg_ex_98( ) iv_user_command = 'FILTER' ).
    cl_abap_unit_assert=>assert_equals( act = ls_result-status-status exp = 'FILTERED' ).
  ...
  METHOD rejects_undeclared_command.
    ...
    APPEND NEW zcl_gg_ex_85( ) TO lt_reports.
    APPEND NEW zcl_gg_ex_86( ) TO lt_reports.
    APPEND NEW zcl_gg_ex_92( ) TO lt_reports.
    APPEND NEW zcl_gg_ex_93( ) TO lt_reports.
    APPEND NEW zcl_gg_ex_94( ) TO lt_reports.
    APPEND NEW zcl_gg_ex_95( ) TO lt_reports.
    APPEND NEW zcl_gg_ex_96( ) TO lt_reports.
    APPEND NEW zcl_gg_ex_98( ) TO lt_reports.
    ...
    APPEND NEW zcl_gg_ex_83( ) TO lt_line_reports.
    APPEND NEW zcl_gg_ex_84( ) TO lt_line_reports.
    LOOP AT lt_line_reports INTO DATA(lo_line_report).
      ...
      cl_abap_unit_assert=>assert_equals( act = ls_line_bad-error exp = 'Invalid list action token' ).
    ENDLOOP.
    zcl_gg_host_runtime=>clear( ).
  ENDMETHOD.
