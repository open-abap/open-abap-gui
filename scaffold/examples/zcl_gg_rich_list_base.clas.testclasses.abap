...
CLASS ltcl_gg_rich_list_base IMPLEMENTATION.
  ...
  METHOD hidden_values.
    DATA(ls_result) = zcl_gg_host=>run( io_report = NEW zcl_gg_ex_084( ) iv_line_index = 2 ).
  ...
  METHOD refresh.
    DATA(ls_result) = zcl_gg_host=>run( io_report = NEW zcl_gg_ex_085( ) iv_user_command = 'REFRESH' ).
  ...
  METHOD modify_lines.
    DATA(ls_result) = zcl_gg_host=>run( io_report = NEW zcl_gg_ex_086( ) iv_user_command = 'MODIFY' ).
  ...
  METHOD find_next.
    DATA(ls_result) = zcl_gg_host=>run( io_report = NEW zcl_gg_ex_093( ) iv_user_command = zif_gg_session_types_v1=>command_find ).
    cl_abap_unit_assert=>assert_true( act = xsdbool( line_exists( ls_result-lines[ table_line = 'Found LH at row 1' ] ) ) ).
  ENDMETHOD.

  METHOD print_view.
    DATA(ls_result) = zcl_gg_host=>run( io_report = NEW zcl_gg_ex_094( ) iv_user_command = 'PRINT_VIEW' ).
    cl_abap_unit_assert=>assert_true( act = xsdbool( ls_result-lines[ 2 ] CS 'PRINT VIEW' ) ).
  ENDMETHOD.

  METHOD download.
    DATA(ls_result) = zcl_gg_host=>run( io_report = NEW zcl_gg_ex_095( ) iv_user_command = 'DOWNLOAD' ).
  ...
  METHOD workbench.
    DATA(ls_result) = zcl_gg_host=>run( io_report = NEW zcl_gg_ex_098( ) iv_user_command = 'FILTER' ).
