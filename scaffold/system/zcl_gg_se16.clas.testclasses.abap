...
CLASS ltcl_gg_se16 IMPLEMENTATION.
  ...
  METHOD displays_bounded_rows.
    ...
    cl_abap_unit_assert=>assert_equals( act = ls_result-values[ name = 'O_RESULT_COUNT' ]-value exp = '2' ).
    cl_abap_unit_assert=>assert_equals( act = ls_result-values[ container = 'TC_RESULT' name = 'CARRID' row = 1 ]-value exp = 'AA' ).
  ...
  METHOD rejects_unknown_table.
    ...
    cl_abap_unit_assert=>assert_equals( act = ls_result-messages[ 1 ]-text exp = 'Table is unknown or not permitted by the data-access policy.' ).
  ENDMETHOD.
