...
CLASS ltcl_gg_se11 IMPLEMENTATION.
  ...
  METHOD metadata.
    ...
    cl_abap_unit_assert=>assert_equals( act = ls_transaction-tcode exp = 'SE11' ).
  ENDMETHOD.
  ...
  METHOD displays_dictionary_table.
    ...
    cl_abap_unit_assert=>assert_equals( act = ls_result-values[ name = 'O_NAME' ]-value exp = 'ZSFLIGHT' ).
    cl_abap_unit_assert=>assert_equals( act = ls_result-values[ container = 'TC_FIELDS' name = 'FIELD_NAME' row = 1 ]-value exp = 'CARRID' ).
  ENDMETHOD.
  ...
  METHOD rejects_unsupported_object.
    ...
    cl_abap_unit_assert=>assert_equals( act = ls_result-values[ name = 'P_OBJECT_TYPE' ]-value exp = 'DOMAIN' ).
    cl_abap_unit_assert=>assert_equals( act = ls_result-screen exp = '0100' ).
    cl_abap_unit_assert=>assert_equals( act = ls_result-messages[ 1 ]-text exp = 'Dictionary object type is not supported.' ).
  ENDMETHOD.
