...
* A disabled command must not react to hover or to being pressed.
  ...
  METHOD renders_top.
    ...
    DATA(lv_untrusted_html) = zcl_gg_workbench_utility=>render_top( iv_title = `</span><style>.wb-appbar{display:none}</style>` ).
    ...
    cl_abap_unit_assert=>assert_true( act = xsdbool( lv_untrusted_html CS '&lt;/span&gt;&lt;style&gt;.wb-appbar{display:none}&lt;/style&gt;' ) ).
    cl_abap_unit_assert=>assert_false( act = xsdbool( lv_untrusted_html CS '</span><style>' ) ).
    ...
    cl_abap_unit_assert=>assert_true( act = xsdbool( lv_dynpro_html CS '>Dynpro</span>' ) ).
