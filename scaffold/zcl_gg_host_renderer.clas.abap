...
CLASS zcl_gg_host_renderer IMPLEMENTATION.
  ...
  METHOD with_navigation.
    ...
    IF is_navigation-modal = abap_true.
      DATA(lv_modal) = |<div class="gg-modal-backdrop" data-navigation-kind="{ zcl_gg_host_html=>escape_attribute( is_navigation-kind ) }" data-navigation-target="{ zcl_gg_host_html=>escape_attribute( is_navigation-target ) }"><section class="gg-modal-panel" role="dialog" aria-modal="true" aria-label="Selection screen { zcl_gg_host_html=>escape_text( is_navigation-target ) }"><header class="gg-modal-header"><span>Selection screen { zcl_gg_host_html=>escape_text( is_navigation-target ) }</span><span class="gg-modal-kind">{ zcl_gg_host_html=>escape_text( is_navigation-kind ) }</span></header>|.
    ...
    ELSE.
      DATA(lv_navigation) = |<nav class="gg-navigation" aria-label="Host navigation" data-navigation-kind="{ zcl_gg_host_html=>escape_attribute( is_navigation-kind ) }" data-navigation-modal="false"><span>Selection screen { zcl_gg_host_html=>escape_text( is_navigation-target ) }</span></nav>|.
...
* box of its own. Only a multi-row range needs the row numbers.
          ...
          DO lv_range_count TIMES.
            ...
            lv_state_attrs = state_attrs(
              is_state    = ls_state
              iv_readonly = xsdbool( ls_state-input = abap_false ) ).
            lv_body = lv_body && |<div class="gg-range-row{ COND string( WHEN ls_element-no_intervals = abap_true THEN ` gg-range-row--single` ELSE `` ) }" data-range-index="{ sy-index }">{ COND string( WHEN lv_range_count > 1 THEN |<span class="gg-range-index" aria-hidden="true">{ sy-index }</span>| ELSE `` ) }<input type="text" id="{ zcl_gg_host_html=>escape_attribute( lv_low_name ) }" name="{ zcl_gg_host_html=>escape_attribute( lv_low_name ) }" value="{ zcl_gg_host_html=>escape_attribute( lv_low_display ) }"{ lv_type_attrs }{ lv_external_attrs }{ lv_state_attrs }>|.
            IF ls_element-no_intervals = abap_false.
              lv_body = lv_body && |<span class="gg-range-to" aria-hidden="true">to</span><input type="text" id="{ zcl_gg_host_html=>escape_attribute( lv_high_name ) }" name="{ zcl_gg_host_html=>escape_attribute( lv_high_name ) }" value="{ zcl_gg_host_html=>escape_attribute( lv_high_display ) }"{ lv_type_attrs }{ lv_high_external_attrs }{ lv_state_attrs }>|.
            ENDIF.
...
* screen would duplicate it and give the page two h1 elements.
  ...
  METHOD render_dynamic_selection.
    ...
    LOOP AT is_selection-fields INTO ls_field.
      ...
      rv_html = rv_html && |<div class="gg-free-selection-row"><label for="gg-free-{ lv_field_name }-low">{ zcl_gg_host_html=>escape_text( lv_field_text ) }</label><select name="gg-free-{ lv_field_name }-SIGN" aria-label="{ zcl_gg_host_html=>escape_attribute( lv_field_text ) } sign"><option value="I"{ lv_sign_i }>Include</option><option value="E"{ lv_sign_e }>Exclude</option></select><select name="gg-free-{ lv_field_name }-OPTION" aria-label="{ zcl_gg_host_html=>escape_attribute( lv_field_text ) } option"><option value="EQ"{ lv_option_eq }>Equals</option><option value="BT"{ lv_option_bt }>Between</option></select><input id="gg-free-{ lv_field_name }-low" type="text" name="gg-free-{ lv_field_name }-LOW" value="{ zcl_gg_host_html=>escape_attribute( ls_field-low ) }" placeholder="Low"><input type="text" name="gg-free-{ lv_field_name }-HIGH" value="{ zcl_gg_host_html=>escape_attribute( ls_field-high ) }" placeholder="High"></div>|.
    ENDLOOP.
