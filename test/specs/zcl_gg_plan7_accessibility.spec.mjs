...
// One test per batch instead of one test over all 92 examples, so the sweep
...
for (let start = 0; start < plan7Examples.length; start += batchSize) {
  const batch = plan7Examples.slice(start, start + batchSize);
  const label = `${batch[0]}-${batch[batch.length - 1]}`;

  test(`PLAN7 — every rendered control has a name and keyboard-visible focus (${label})`, async ({page, host}) => {
    for (const number of batch) {
      await auditExample(page, host, number);
    }
  });
}
