export function selectInterfaces(ir) {
  ...
  if (ir.screenMetadata?.screens?.length && ir.programKind === "report") interfaces.add("zif_gg_screen_provider_v1");
  if ((ir.routines ?? []).some((routine) => /^ON_CTMENU(?:_|$)/i.test(String(routine.name ?? "")))) interfaces.add("zif_gg_context_menu_v1");
